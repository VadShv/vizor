...
r.post("/datasets/:id/share", authMiddleware, async (c) => {
  ...
  const ds = await db.select().from(datasets).where(eq(datasets.id, datasetId)).get();
  ...
  const existing = await db.select().from(shares).where(eq(shares.datasetId, datasetId)).get();
...
r.get("/shared/:token", async (c) => {
  ...
  const share = await db.select().from(shares).where(eq(shares.token, token)).get();
  ...
  const ds = await db.select().from(datasets).where(eq(datasets.id, share.datasetId)).get();
